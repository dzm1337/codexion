...
/*   Updated: 2026/07/02 22:14:14 by dde-paul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	setup_deadline(t_dongle *dongle, t_coder *coder)
{
	long	last_compile;
	...
	coder->deadline = last_compile + coder->data->time_to_burnout;
	add_to_queue(dongle, coder);
}

static long	wait_loop(t_dongle *dongle, t_coder *coder, long cooldown)
{
	struct timespec	ts;
	long			now;
	long			wait_ms;
	...
	while (1)
	{
		...
		if (check_stop_and_relock(dongle, coder))
			return (-1);
		now = get_current_time();
...
int	wait_for_dongle(t_dongle *dongle, t_coder *coder)
{
	...
	setup_deadline(dongle, coder);
	now = wait_loop(dongle, coder, coder->data->dongle_cooldown);
	if (now == -1)
		return (0);
	grant_dongle(dongle, coder, now);
	return (1);
}
